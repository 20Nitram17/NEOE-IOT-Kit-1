...
NEOE IOT-Kit 1 "Room Air Monitoring with MQ135, NodeMCU & Blynk"
...
for (int thisReading = 0; thisReading < numReadings; thisReading++) { readings[thisReading] = 0; } Blynk.begin(auth, ssid, pass); } void loop() { /* Ensure that Blynk reconnects in case of lost connection */ if(!Blynk.connected() ) { Blynk.connect(); } Blynk.run(); /* Read sensor data and create average */ total = total - readings[readIndex]; readings[readIndex] = analogRead(_pin); total = total + readings[readIndex]; readIndex = readIndex + 1; if (readIndex >= numReadings) {
readIndex = 0;
}
...
if (LEDgreen < 0) LEDgreen = 0; /* Blynk notification if PPM exceeds 2000 */ if (ppm > 2000) {
if (notified == 0) Blynk.notify("Please open window / bitte lüften");
notified = 1;
}
else
{
